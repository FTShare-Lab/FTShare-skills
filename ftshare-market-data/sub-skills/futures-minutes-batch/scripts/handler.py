...
ENDPOINT = "/api/v3/market/data/futures_minutes/batch"
...
def main():
    _require_api_key()
    parser = argparse.ArgumentParser(description="批量查询期货合约历史分钟 K 线")
    ...
    params = [("symbols", symbol) for value in args.symbols for symbol in value.split(",") if symbol]
    params.extend((key, value) for key, value in vars(args).items() if key != "symbols" and value is not None)
