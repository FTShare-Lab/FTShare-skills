...
BASE_URL=os.environ.get("FTSHARE_BASE_URL","https://market.ft.tech/gateway").rstrip("/"); ENDPOINT="/api/v1/market/data/stk-code-change"
...
def main():
 p=argparse.ArgumentParser(description="查询 A 股代码变更"); p.add_argument("--trade-code",required=True); p.add_argument("--start-date"); p.add_argument("--end-date"); a=p.parse_args()
 if not a.trade_code.strip(): p.error("trade-code 不能为空")
 if a.start_date and a.end_date and a.start_date>a.end_date: p.error("start-date 不得晚于 end-date")
 params={"trade_code":a.trade_code};
 if a.start_date: params["start_date"]=a.start_date
 if a.end_date: params["end_date"]=a.end_date
