...
ENDPOINT = '/api/v1/market/data/daec/stocks/all'
...
def main():
    key = _require_api_key(); parser = argparse.ArgumentParser(description='A股行情列表')
    ...
    if args.filter is not None: params["filter"] = args.filter
    if args.order_by is not None: params["order_by"] = args.order_by
