...
} if os.environ.get("FTSHARE_API_KEY") else {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="查询股票历史列表")
    parser.add_argument("--trade-date", dest="trade_date", required=True, help="交易日，格式 YYYYMMDD")
    parser.add_argument("--code", help="股票代码，可为 600000.SH、600000.XSHG 或纯六位数字")
    ...
    request = urllib.request.Request(
        BASE_URL + ENDPOINT + "?" + urllib.parse.urlencode(params),
        headers={**_REQUEST_HEADERS, "FTSHARE_API_KEY": key, "X-Client-Name": "ft-claw"},
        method="GET",
    )
    try:
