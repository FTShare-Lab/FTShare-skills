...
def main():
    ...
    parser = argparse.ArgumentParser(description="查询 A 股研报列表")
    ...
    if args.stock_code is None and (args.type is None or args.start_date is None):
        parser.error("未提供 stock-code 时，type 与 start-date 必须同时提供")
    if args.stock_code is not None and args.type is not None and args.type.lower() != "stockreport":
        parser.error("按 stock-code 查询时 type 必须为 StockReport")
    params = {"page": args.page, "page_size": args.page_size}
    for name in ("stock_code", "type", "start_date", "end_date"):
